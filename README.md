...
| **בעלים (owner)** | מחובר לסירה אחת, רואה בלבד (read-only) את נתוני הסירה שלו |
...
לכל סירה: פרטים בסיסיים, תחזוקה, הזמנות/צ'רטרים, דוחות כספיים (הכנסות/הוצאות) ומסמכים (ביטוח/רישיון/רישום) עם העלאת קבצים ל-Supabase Storage.
...
בפרויקט Supabase שלכם: **SQL Editor** → הדביקו והריצו את התוכן של `supabase/migrations/0001_init.sql`.

הקובץ יוצר:
- טבלאות: `profiles`, `boats`, `maintenance_records`, `bookings`, `financial_records`, `documents`
