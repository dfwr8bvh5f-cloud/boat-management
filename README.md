...
| **בעלים (owner)** | מחובר לסירה אחת, רואה רק רשומות **שאושרו** על ידי הניהול בכל מודול שתומך באישור (כרגע: הוצאות, תקלות טכניות) |
...
מודולי ההוצאות והתקלות הטכניות מיישמים זרימת אישור אמיתית: רשומה שנוצרת על ידי קפטן מקבלת סטטוס `pending`, רשומה שנוצרת על ידי ניהול מאושרת אוטומטית (`approved`). ניהול יכול לאשר רשומה ממתינה (כפתור "אשר"). בעלים רואה רק רשומות מאושרות — האכיפה היא ב-RLS ולא רק ב-UI, כולל על תמונות/קבצים מצורפים ב-Storage. Trigger משותף בשם `prevent_self_approval` חוסם קפטן מלאשר את עצמו גם דרך קריאת API ישירה (מחובר לכל טבלה שתומכת באישור).
...
מודולי הזמנות/מסמכים עדיין לא עברו לזרימת האישור הזו (בעלים רואה בהם הכל, read-only) — זה מתוכנן למודולים הבאים.

## מודולים

לכל סירה: פרטים בסיסיים, הזמנות/צ'רטרים, מסמכים (ביטוח/רישיון/רישום) עם העלאת קבצים ל-Supabase Storage, תקציב+הוצאות (קטגוריות, תתי-קטגוריות, קבלות מצולמות, זרימת אישור), ותקלות טכניות + ציוד בטיחות (סיווג, אזור, ספק, הצעת מחיר, סטטוס תפעולי, זרימת אישור).
...
בפרויקט Supabase שלכם: **SQL Editor** → הריצו **לפי הסדר** את שלושת הקבצים תחת `supabase/migrations/` (כל אחד בנפרד, כ-query חדש):
