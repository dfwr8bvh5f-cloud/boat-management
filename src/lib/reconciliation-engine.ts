...
const BANK_FEE_PATTERN =
  /\b(bank\s*(commission|fee|charge)|card\s*fee|sepa\s*(fee|charge)|transfer\s*fee|wire\s*fee|account\s*maintenance|maintenance\s*fee|service\s*charge|commission)\b|עמלת?\s*(בנק|כרטיס|העברה|ניהול)|דמי\s*ניהול/i;
