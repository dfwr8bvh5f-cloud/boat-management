...
export async function updateBookingGuest(boatId: string, guestId: string, formData: FormData): Promise<{ error: string | null }> {
  try {
    const supabase = await createClient();
    ...
    if (file instanceof File && file.size > 0) {
      const { data: existing } = await supabase.from("booking_guests").select("photo_path").eq("id", guestId).single();
