...
export function WeeklyEngineReportForm({
  ...
  return (
    <div className="rounded-xl border border-fleet-border bg-white p-4">
      ...
      {canEdit ? (
        <form
          action={upsertWeeklyEngineReport.bind(null, boatId, weekOf)}
          className="grid grid-cols-2 gap-3 sm:grid-cols-4"
        >
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-fleet-ink">{t("weekly_main_engine_hours")}</label>
            <input
              name="main_engine_hours"
              type="number"
              step="0.1"
              defaultValue={existing?.main_engine_hours ?? ""}
              className={inputClass}
            />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-fleet-ink">{t("weekly_generator_main_hours")}</label>
            <input
              name="generator_main_hours"
              type="number"
              step="0.1"
              defaultValue={existing?.generator_main_hours ?? ""}
              className={inputClass}
            />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-fleet-ink">{t("weekly_generator_secondary_hours")}</label>
            <input
              name="generator_secondary_hours"
              type="number"
              step="0.1"
              defaultValue={existing?.generator_secondary_hours ?? ""}
              className={inputClass}
            />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className="text-xs text-fleet-ink">{t("weekly_watermaker_hours")}</label>
            <input
              name="watermaker_hours"
              type="number"
              step="0.1"
              defaultValue={existing?.watermaker_hours ?? ""}
              className={inputClass}
            />
          </div>
        ...
        <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-4">
          <div>
            ...
            <dd className="font-medium text-fleet-navy">{existing.main_engine_hours ?? "—"}</dd>
          </div>
          <div>
            <dt className="text-[11px] text-fleet-ink">{t("weekly_generator_main_hours")}</dt>
            <dd className="font-medium text-fleet-navy">{existing.generator_main_hours ?? "—"}</dd>
          </div>
          <div>
            <dt className="text-[11px] text-fleet-ink">{t("weekly_generator_secondary_hours")}</dt>
            <dd className="font-medium text-fleet-navy">{existing.generator_secondary_hours ?? "—"}</dd>
          </div>
          <div>
            <dt className="text-[11px] text-fleet-ink">{t("weekly_watermaker_hours")}</dt>
            <dd className="font-medium text-fleet-navy">{existing.watermaker_hours ?? "—"}</dd>
          </div>
