-- ============================================================================
-- Staff: an ID card / passport number + scanned document (image or PDF),
-- same pattern as the existing resume upload. Additive only - the
-- salary-masking view is redefined as a superset of its previous columns.
-- ============================================================================

alter table public.staff add column if not exists id_number text;
alter table public.staff add column if not exists id_document_path text;

-- cascade: the storage RLS policy below reads through this view, so it gets
-- dropped along with it and must be recreated after.
  ...
  using (
    ...
    and (
      ...
      or exists (
        ...
        where s.photo_path = storage.objects.name or s.resume_path = storage.objects.name or s.id_document_path = storage.objects.name
      )
    )
  );
